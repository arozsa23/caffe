...
void InnerProductLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (beta != 0.0) {
    ...
    const Dtype* top_diff = top[0]->cpu_diff();
    Dtype* mutable_top_diff = top[0]->mutable_cpu_diff();
    ...
    for (int j = 0; j < batch_size; ++j) {
      Dtype diff_l2 = std::sqrt(caffe_cpu_dot(element_size, top_diff + j * element_size,
                                top_diff + j * element_size));
    ...
    for (int j = 0; j < batch_size; ++j) {
      if (l2_norms[j] != 0.0) {
        ...
        caffe_scal(element_size, scale, mutable_top_diff + j * element_size);
      }
    }
  }
  // *****************************************************************************
